...
// For 'most_violated_constraint' functionality in structured search,
// we need to know the ground truth boxes to calculate the loss 
static double* ref_boxes;
static int num_ref_boxes;
...
// it adds the loss-function in scoring the boxes, apart from that it's
// the same as pyramid_search(...)
//
// INPUT: int argnumpoints,     : number of data points
//        int width, height     : width and height of image
//        double* argxpos,      : x-coordinate of every point
...
//        double* weightsdata   : coordinates of ground truth boxes in 1D vector
//
// OUTPUT: Box outputBox        : box in [left,top,right,bottom,score] format
Box pyramid_search_structured(int argnumdatapts, int argwidth, int argheight, 
                   double* argxpos, double* argypos, double* argclst,
                   int argnumclusters, int argnumlevels, double* argweightdata, 
                   int num_gt_boxes, double* gt_box) {

        ref_boxes = gt_box;
        num_ref_boxes = num_gt_boxes;
        Box outputBox = pyramid_search(argnumdatapts, argwidth, argheight, 
                                       argxpos, argypos, argclst, 
                                       argnumclusters, argnumlevels, argweightdata);
        return outputBox;
}
